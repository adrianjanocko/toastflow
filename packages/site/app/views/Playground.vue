...
import type { PlaygroundButton } from "~/types/playground-types";
...
const giscusRuntime = (runtimeConfig.public.giscus ??
  {}) as PublicRuntimeGiscus;
const giscusConfig = {
  repo: giscusRuntime.repo ?? "",
  repoId: giscusRuntime.repoId ?? "",
  category: giscusRuntime.category ?? "",
  categoryId: giscusRuntime.categoryId ?? "",
  mapping: (giscusRuntime.mapping ?? "pathname") as Mapping,
  theme: giscusRuntime.theme ?? "light",
  strict: (giscusRuntime.strict ?? "0") as BooleanString,
};
...
const giscusTheme = computed(function () {
  const lightTheme = giscusRuntime.themeLight ?? giscusConfig.theme ?? "light";
  const darkTheme = giscusRuntime.themeDark ?? "dark_dimmed";
...
function createPlaygroundButton(
  partial?: Partial<PlaygroundButton>,
): PlaygroundButton {
  buttonCounter += 1;
  return {
...
const playgroundButtons = ref<PlaygroundButton[]>([
  createPlaygroundButton({ value: "Undo", dismissAfterClick: true }),
...
function getActiveOnboardingTargetElement() {
  ...
  if (activeOnboardingStep.value.id === "actions") {
    return isDesktopLayout.value
      ? actionsDesktopRef.value
      : actionsMobileRef.value;
  }
...
function updateOnboardingSpotlight() {
  ...
  const paddedRight = Math.min(
    viewportWidth,
    rect.right + ONBOARDING_SPOTLIGHT_PADDING,
  );
  let paddedBottom = Math.min(
    viewportHeight,
    rect.bottom + ONBOARDING_SPOTLIGHT_PADDING,
  );
  ...
  const buttons = playgroundButtons.value
    ...
    .map(function (button): ToastButton {
      const base = {
        ...
        dismissAfterClick: button.dismissAfterClick,
...
function updateLast() {
  ...
  const updatedTitle = resolveContent(
    title.value,
    "Updated toast",
    fallbackTitle.value,
  );
...
function removePlaygroundButton(id: string) {
  playgroundButtons.value = playgroundButtons.value.filter(
    (button) => button.id !== id,
  );
}

function updatePlaygroundButton(
  id: string,
  updates: Partial<PlaygroundButton>,
) {
...
function hydrateFromQuery() {
  ...
  try {
    ...
    console.warn(
      "[toastflow playground] Failed to parse state query param",
      error,
    );
    return;
  }
    ...
    const safeButtons = (state.buttons as PlaygroundButton[])
      .map(function (button) {
        return createPlaygroundButton({
          ...
          dismissAfterClick: Boolean(button.dismissAfterClick),
        });
      })
...
function buildButtonsSnippetLines() {
  ...
  filteredButtons.forEach(function (button) {
    ...
    if (button.dismissOnClick) {
      lines.push("      dismissOnClick: true,");
    }
    if (button.dismissAfterClick) {
      lines.push("      dismissAfterClick: true,");
    }
...
const codeSnippet = computed(function () {
  ...
  const content = getContentForType(
    targetType === "loading" ? "loading" : targetType,
  );
  const optionLines = buildOptionsLines(
    content,
    targetType === "loading" ? "loading" : targetType,
  );
...
watch(
  [isOnboardingOpen, onboardingStepIndex, isDesktopLayout],
  function ([open]) {
    if (!open) {
      onboardingSpotlightRect.value = null;
      return;
...
<template>
  ...
  >
    <div class="flex flex-col gap-3">
      <div class="flex flex-wrap items-center justify-between gap-3">
        ...
        >
          <div
            v-for="shortcut in shortcutKeys"
            :key="shortcut.key"
      ...
      <div class="flex flex-wrap items-center justify-between gap-2">
        ...
        <div class="flex flex-wrap items-center gap-2">
          <Button
            ...
            @click="copyShareLink"
          >
            <UIcon name="i-tabler-share-2" class="h-4 w-4" />
            {{ copyShareLabel }}
          </Button>
          <Button
          ...
          >
            <UIcon name="i-tabler-message-circle" class="h-4 w-4" />
            Feedback
          </Button>
        </div>
      </div>
    ...
    >
      <div class="flex flex-wrap items-center justify-between gap-3 mb-2">
        ...
        >
          <UIcon
            name="i-tabler-terminal"
            class="h-[1.15rem] w-[1.15rem] opacity-75"
          />
          Live code
        ...
        >
          <UIcon name="i-tabler-copy" class="h-4 w-4" />
          {{ copyCodeLabel }}
        </Button>
      </div>
      <ClientOnly>
  ...
  <Teleport to="body">
    <div
      v-if="isOnboardingOpen && activeOnboardingStep"
      class="fixed inset-0 z-85"
    >
      <div class="absolute inset-0 pointer-events-none">
        <div
          v-if="!onboardingSpotlightRect"
          class="onboarding-overlay-fallback"
        />
        <div
          class="onboarding-spotlight-mask"
          :style="onboardingSpotlightMaskStyle"
        />
      </div>
  ...
  <Modal v-model="isLogModalOpen" title="Event log">
    <div class="grid gap-4">
      ...
      >
        <span v-if="eventLog.length"
          >Newest first - {{ eventLog.length }} events</span
        >
